...
<form method="post" action="/edwell/Deletesentmsg">
<table border=3>
<tr><th>Delete</th><th>Receiver id</th><th>Subject</th><th>Message</th><th>Date</th></tr>
 <%  hs=request.getSession();
 String ui=(String)hs.getAttribute("userinfo");
 System.out.println(ui);
 String strsql="Select msgid,receiverid,msgtext,subject,date from message where senderid=? and Sstatus=?";
 ...
 try
 {
	 ...
	 while(rs.next())
	 { %>
	 <tr><td><input type="checkbox" name="chkmsg" value="<%=rs.getInt("msgid") %>"></td><td><%=rs.getString("receiverid") %><td><%=rs.getString("subject") %></td><td><%= rs.getString("msgtext") %></td><td><%=rs.getDate("date") %></td></tr>
